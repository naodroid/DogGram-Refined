...
public actor ImagesRepositoryImpl: ImagesRepository {
    ...
    public func downloadProfileImage(userID: String) async throws -> UIImage {
        let path = getProfileImagePath(userID: userID)
    ...
    public func uploadPostImage(postID: String,
                         image: UIImage
    ) async throws {
    ...
    /// Download the image of the post
    /// When failed, an error will be thrown
    public func downloadPostImage(postID: String) async throws -> UIImage {
        let path = getPostImagePath(postID: postID)
        return try await downloadImage(path: path)
    }
    /// Delete the image of the post
    public func deletePostImage(postID: String) async throws {
        let path = getPostImagePath(postID: postID)
        try await deleteImage(path: path)
    }
